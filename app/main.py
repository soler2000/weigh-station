...
engine = create_engine("sqlite:///./data/weigh.db", future=True, connect_args={"check_same_thread": False})
...
def update_variant(variant_id: int = Path(..., ge=1), v: VariantIn = ...):
    with Session() as s:
...
def delete_variant(variant_id: int = Path(..., ge=1)):
    with Session() as s:
        row = s.get(Variant, variant_id)
        if not row: raise HTTPException(404, "Variant not found")
